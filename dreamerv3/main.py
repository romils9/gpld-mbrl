...
from dreamerv3.utils.tqdm_logger import TqdmOutput  # add this import
...
def make_logger(config):
  ...
  total_steps = int(config.run.get('steps', 0)) if isinstance(config.run, dict) else int(config.run.steps)
  if total_steps > 0 and int(getattr(config, 'replica', 0)) == 0:  # show only on replica 0
