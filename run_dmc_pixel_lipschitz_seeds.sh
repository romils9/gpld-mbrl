...
#   PRIOR_LAM=0.01 POST_LAM=0.01 DECAY_TYPE=step ./run_dmc_pixel_lipschitz_seeds.sh
#   LOGDIR_ROOT=./logdir/vision SEEDS="0 1 2 3 4" ./run_dmc_pixel_lipschitz_seeds.sh
#
# Decay types:
...
REPEAT="${REPEAT:-2}"
ENV_NAME="${ENV_NAME:-dmc_cheetah_run}"
LOGDIR_ROOT="${LOGDIR_ROOT:-./logdir/gpld_vision}"

# ============================================================================
# Lipschitz constraint parameters (gradient penalty)
# ============================================================================
PRIOR_LAM="${PRIOR_LAM:-0.0}"          # Prior gradient penalty weight
POST_LAM="${POST_LAM:-0.5}"            # Posterior gradient penalty weight
...
START_STEP="${START_STEP:-0}"        # Start step for decay

# Parameters for linear/exponential/cosine decay
DECAY_STEPS="${DECAY_STEPS:-100000}"     # Steps over which to decay
MIN_LAM="${MIN_LAM:-0.01}"                # Minimum lambda value (floor)
...
if awk -v a="$PRIOR_LAM" -v b="$POST_LAM" 'BEGIN{exit !((a+0)>0 || (b+0)>0)}'; then
  ...
  if [[ "$DECAY_TYPE" != "none" ]]; then
    EXP_SUFFIX="${EXP_SUFFIX}_${DECAY_TYPE}_GP_start${START_STEP}"
  fi
else
  EXP_SUFFIX="baseline"
fi
...
build_decay_args() {
  ...
  case "$DECAY_TYPE" in
    ...
    step)
      ...
      decay_args="${decay_args} --agent.dyn.rssm.gp.min_lam ${MIN_LAM}"
      decay_args="${decay_args} --agent.dyn.rssm.gp.start_step ${START_STEP}"
      ;;
